...
class _DebtEditor extends ConsumerState<DebtEditor>
{
  ...
  bool calculateAmount()
  {
    ...
    try
    {
      widget.debt.amount = TeXParser(_amountController.currentEditingValue()).parse().evaluate(EvaluationType.REAL, ContextModel());
      _amountController.updateValue(Parser().parse(widget.debt.amount.toStringAsFixed(2)));
      widget.debt.amount = TeXParser(_amountController.currentEditingValue()).parse().evaluate(EvaluationType.REAL, ContextModel());
    ...
    {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("An amount could not be parsed. ${widget.debt.memo.isNotEmpty ? " Debt: ${widget.debt.memo}" : ""}")));
