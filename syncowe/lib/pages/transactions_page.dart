...
class _TransactionPage extends ConsumerState<TransactionsPage> {
  ...
  double getPersonalChange(Transaction transaction, String user) {
    if (transaction.payer == user) {
      ...
      return transaction.total - (payerDebt?.amount ?? 0);
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final tripUsers = ref.watch(tripUsersProvider);
    ...
    return FirestorePagination(
        ...
        itemBuilder: (context, snapshot, index) {
          ...
          User? payer = tripUsers[transaction.payer];
          ...
          return Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(payer?.displayName ?? payer?.email ?? "Unknown"),
              leading: Text(
