...
@riverpod
syncowe.Transaction? currentTransaction(CurrentTransactionRef ref)
{
...
@riverpod
Future<syncowe.Transaction?> currentTransactionAsync(CurrentTransactionAsyncRef ref) async
{
  String? currentId = ref.watch(currentTransactionIdProvider);
  String? tripId = ref.watch(currentTripIdProvider);

  Map<String, syncowe.Transaction>? loadedTransactions = ref.watch(loadedTransactionsProvider);
  var tripFirestoreService = ref.read(tripFirestoreServiceProvider.notifier);

  if(currentId != null && tripId != null)
  {
    return loadedTransactions?[currentId] ?? await tripFirestoreService.getTransaction(tripId, currentId);
  }
  else
  {
    return null;
