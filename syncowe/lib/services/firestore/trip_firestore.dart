...
class TripFirestoreService extends _$TripFirestoreService {
  ...
  final CollectionReference _trips = FirebaseFirestore.instance
      .collection("Trips")
      .withConverter<Trip>(
          fromFirestore: Trip.fromFirestore,
          toFirestore: (trip, options) => (trip).toJson());

  Stream<QuerySnapshot> listenToTrips({bool archived = false}) {
    return _trips
  ...
  Stream<Trip?> listenToTrip(String id) {
    return _trips
        .doc(id)
        .snapshots()
        .map((snapshot) => snapshot.data() as Trip);
  }
  ...
  CollectionReference transactions(String tripId) {
    return getTripDoc(tripId)
        ...
        .withConverter<syncowe_transaction.Transaction>(
            fromFirestore: syncowe_transaction.Transaction.fromFirestore,
            toFirestore: (transaction, options) => transaction.toJson());
  }

  Stream<syncowe_transaction.Transaction?> listenToTransaction(
      String tripId, String id) {
    return transactions(tripId)
        .doc(id)
        .snapshots()
        .map((snapshot) => snapshot.data() as syncowe_transaction.Transaction);
  }

  Future<syncowe_transaction.Transaction?> getTransaction(
      String tripId, String id) async {
  ...
  Future<String> addOrUpdateTransaction(
      syncowe_transaction.Transaction transaction,
      String tripId,
      String? id) async {
    if (id == null) {
  ...
  CollectionReference reimbursements(String tripId) {
    return getTripDoc(tripId)
        ...
        .withConverter<Reimbursement>(
            fromFirestore: Reimbursement.fromFirestore,
            toFirestore: (reimbursement, options) => reimbursement.toJson());
  }

  Future<void> addOrUpdateReimbursement(
      Reimbursement reimbursement, String tripId, String? id) async {
    if (id == null) {
  ...
  CollectionReference _overallDebts(String tripId) {
    return _trips
        .doc(tripId)
        .collection("OverallDebts")
        .withConverter<DebtPair>(
  ...
  CollectionReference _overallDebtSummary(String tripId, String debtPairId) {
    return _overallDebts(tripId)
        .doc(debtPairId)
        .collection("OverallDebtSummary")
        .withConverter<OverallDebtSummary>(
            fromFirestore: OverallDebtSummary.fromFirestore,
            toFirestore: (summary, options) => summary.toJson());
  }

  Stream<QuerySnapshot> listenToOverallDebtSummary(
      String tripId, String debtPairId,
      {bool archived = false}) {
    return _overallDebtSummary(tripId, debtPairId)
        .where(NameofOverallDebtSummary.fieldArchived, isEqualTo: archived)
        .snapshots();
  }
}
